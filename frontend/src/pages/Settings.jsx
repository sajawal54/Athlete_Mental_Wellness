...
export default function Settings() {
  ...
  const [userProfile, setUserProfile] = useState({ username: '', email: '', avatar: null });
  
  // Functional Theme State (Persisted in localStorage)
  const [isDarkMode, setIsDarkMode] = useState(() => {
    return localStorage.getItem('app-theme') === 'dark';
  });

  // Dummy State for future Backend-connected Notifications
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 p-4 sm:p-6 dark:bg-slate-900 min-h-screen">
      ...
      {/* 1. USER SUMMARY CARD (Dynamic Avatar Display) */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-5 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-4 transition-colors">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl overflow-hidden bg-gradient-to-tr from-indigo-600 to-violet-600 text-white font-black text-lg flex items-center justify-center shadow-lg shadow-indigo-600/20 shrink-0 border border-slate-100 dark:border-slate-800">
            {userProfile.avatar ? (
              <img 
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-6 shadow-xs space-y-4 transition-colors">
        <h3 className="text-xs font-extrabold uppercase tracking-wider text-slate-800 dark:text-white flex items-center gap-2">
          {isDarkMode ? <MoonIcon className="w-4 h-4 text-indigo-500" /> : <SunIcon className="w-4 h-4 text-amber-500" />}
          Appearance & Theme
        </h3>

        <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800">
          <div>
            <p className="text-xs font-bold text-slate-800 dark:text-slate-200">Dark Background Mode</p>
            <p className="text-[11px] text-slate-400">Switch between standard light interface and contrast dark background.</p>
          </div>

          <button 
            onClick={toggleTheme}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none cursor-pointer ${isDarkMode ? 'bg-indigo-600' : 'bg-slate-300'}`}
          >
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-6 shadow-xs space-y-4 transition-colors">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-extrabold uppercase tracking-wider text-slate-800 dark:text-white flex items-center gap-2">
            <BellIcon className="w-4 h-4 text-indigo-500" /> Notifications & Reminders
          </h3>
          <span className="text-[10px] font-bold text-slate-400 uppercase bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded">Future API Sync</span>
        </div>

        <div className="space-y-3 pt-2 border-t border-slate-100 dark:border-slate-800">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-700 dark:text-slate-300">Email Updates & Weekly Digest</span>
            <input 
              type="checkbox" 
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-700 dark:text-slate-300">Daily Wellness & Goal Reminders</span>
            <input 
              type="checkbox" 
              checked={notifications.goalReminders}
              onChange={() => setNotifications(p => ({ ...p, goalReminders: !p.goalReminders }))}
              className="w-4 h-4 accent-indigo-600 rounded cursor-pointer"
            />
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-6 shadow-xs space-y-4 transition-colors">
        <h3 className="text-xs font-extrabold uppercase tracking-wider text-slate-800 dark:text-white flex items-center gap-2">
          <ShieldCheckIcon className="w-4 h-4 text-indigo-500" /> Account Security
        </h3>
        <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800">
          <div>
            <p className="text-xs font-bold text-slate-800 dark:text-slate-200">Password Update</p>
            <p className="text-[11px] text-slate-400">Change your password directly from your main profile edit drawer.</p>
          </div>
          <button 
            onClick={() => navigate('/profile')}
